...
export default async function handler(
    ...
    try {
        ...
        const r = req.query.r === undefined ? 0 : parseFloat(typeof req.query.r === 'string' ? req.query.r : req.query.r[0]);
        ...
        res.status(200).json(
            pointsArray.filter((el) => r === 0 || haversineDistance(lat, lon, el.latitude, el.longitude) <= r).map((el) => {
                return {
                    id: el.id,
